...
void itoa(int i, char a[9]) {
    char table[] = "0123456789abcdef";
    for (int j = 0; j < 8; j++) {
        a[7 - j] = table[(i >> (4 * j)) & 0xf];
    }
    a[8] = '\0';
}
...
int main(int argc, char* argv[]) {
    ...
    if (std::string(argv[1]) == "generate") {
        ...
        printf("* Size of H is: %ld\n", keypair.pk.H_query.size());
        printf("* Size of K is: %ld\n", keypair.pk.A_query.size());
        ...
        leaf = hash256<HashT>(num);
        root = leaf;
        address = 0;
        for (int i = 0; i < tree_depth; i++) {
            itoa(i+1, num);

            path[tree_depth-1-i] = hash256<HashT>(num);
            address_bits[i] = 0;

            libff::bit_vector temp = root;
            temp.insert(temp.end(), path[tree_depth-1-i].begin(), path[tree_depth-1-i].end());
            root = HashT::get_hash(temp);
        }
