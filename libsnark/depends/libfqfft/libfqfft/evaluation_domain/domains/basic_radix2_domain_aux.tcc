...
void _basic_parallel_radix2_FFT(std::vector<libff::alt_bn128_Fr> &a, const libff::alt_bn128_Fr &omega)
{
    const size_t n = a.size(), logn = log2(n);
    if (n != (1u << logn)) throw DomainSizeException("expected n == (1u << logn)");
    ...
    if (logn >= 18 && logn <= 27) {
        libff::alt_bn128_Fr omega2 = libff::alt_bn128_Fr::root_of_unity;
        for (int i = libff::alt_bn128_Fr::s; i > logn; i--) omega2 *= omega2;

        if (omega == omega2) use_hw = true;
        else if (omega == omega2.inverse()) {
            use_hw = true;
            is_inv = true;
        } 
    }
    if(use_hw) {
        ...
        uint64_t *ntt_i = (uint64_t *)(hw_device->hw_ntt_i());
        for (int i = 0; i < n; i++) {
            memcpy(ntt_i, a[i].mont_repr.data, 32);
            ntt_i += 4;
        }
        hw_device->hw_ntt(logn, is_inv);
        const uint64_t *ntt_o = (const uint64_t *)(hw_device->hw_ntt_o());
        for (int i = 0; i < n; i++) {
            memcpy(a[i].mont_repr.data, ntt_o, 32);
            ntt_o += 4;
        }
        return;
    }
