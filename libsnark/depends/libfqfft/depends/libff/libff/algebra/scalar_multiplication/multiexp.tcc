...
template<typename T, typename FieldT, multi_exp_method Method>
libff::alt_bn128_G1 multi_exp(typename std::vector<libff::alt_bn128_G1>::const_iterator vec_start,
            typename std::vector<libff::alt_bn128_G1>::const_iterator vec_end,
            typename std::vector<libff::alt_bn128_Fr>::const_iterator scalar_start,
            typename std::vector<libff::alt_bn128_Fr>::const_iterator scalar_end,
            const size_t chunks)
{
    ...
    if (total >= (1lu << 18) && total <= (1lu << 27)) {
        ...
        uint64_t *msm_point_i = (uint64_t *)(hw_device->hw_msm_point_i());
        uint64_t N = 0;
        for (uint64_t i = 0; i < total; i++) {
            if (!vec_start[i].is_zero()) {
                memcpy(msm_scalar_i, scalar_start[i].mont_repr.data, 32);
                msm_scalar_i += 4;
                memcpy(msm_point_i, vec_start[i].X.mont_repr.data, 32);
                msm_point_i += 4;
                memcpy(msm_point_i, vec_start[i].Y.mont_repr.data, 32);
                msm_point_i += 4;
                N++;
            }
        }
        hw_device->hw_msm(N);
        const uint64_t *msm_o = (const uint64_t *)(hw_device->hw_msm_o());
        libff::alt_bn128_G1 result;
        memcpy(result.X.mont_repr.data, msm_o, 32);
        msm_o += 4;
        memcpy(result.Y.mont_repr.data, msm_o, 32);
        msm_o += 4;
        memcpy(result.Z.mont_repr.data, msm_o, 32);
    ...
    if ((total < chunks) || (chunks == 1))
    {
        ...
        return multi_exp_inner<libff::alt_bn128_G1, libff::alt_bn128_Fr, Method>(
            vec_start, vec_end, scalar_start, scalar_end);
    }

    const size_t one = total/chunks;

    std::vector<libff::alt_bn128_G1> partial(chunks, libff::alt_bn128_G1::zero());
...
#pragma omp parallel for
    ...
    {
        partial[i] = multi_exp_inner<libff::alt_bn128_G1, libff::alt_bn128_Fr, Method>(
             vec_start + i*one,
    ...
    libff::alt_bn128_G1 final = libff::alt_bn128_G1::zero();
...
template<typename T, typename FieldT, multi_exp_method Method>
typename std::enable_if<!(std::is_same<T, libff::alt_bn128_G1>().value && std::is_same<FieldT, libff::alt_bn128_Fr>().value), T>::type multi_exp(typename std::vector<T>::const_iterator vec_start,
            typename std::vector<T>::const_iterator vec_end,
            typename std::vector<FieldT>::const_iterator scalar_start,
            typename std::vector<FieldT>::const_iterator scalar_end,
            const size_t chunks)
...
template<typename T, typename FieldT, multi_exp_method Method>
libff::alt_bn128_G1 multi_exp_with_mixed_addition(typename std::vector<libff::alt_bn128_G1>::const_iterator vec_start,
                                typename std::vector<libff::alt_bn128_G1>::const_iterator vec_end,
                                typename std::vector<libff::alt_bn128_Fr>::const_iterator scalar_start,
                                typename std::vector<libff::alt_bn128_Fr>::const_iterator scalar_end,
                                const size_t chunks)
{
    ...
    const libff::alt_bn128_Fr zero = libff::alt_bn128_Fr::zero();
    const libff::alt_bn128_Fr one = libff::alt_bn128_Fr::one();
    std::vector<libff::alt_bn128_Fr> p;
    std::vector<libff::alt_bn128_G1> g;

    libff::alt_bn128_G1 acc = libff::alt_bn128_G1::zero();
    ...
    return acc + multi_exp<libff::alt_bn128_G1, libff::alt_bn128_Fr, Method>(g.begin(), g.end(), p.begin(), p.end(), chunks);
}

template<typename T, typename FieldT, multi_exp_method Method>
typename std::enable_if<!(std::is_same<T, libff::alt_bn128_G1>().value && std::is_same<FieldT, libff::alt_bn128_Fr>().value), T>::type multi_exp_with_mixed_addition(typename std::vector<T>::const_iterator vec_start,
                                typename std::vector<T>::const_iterator vec_end,
                                typename std::vector<FieldT>::const_iterator scalar_start,
                                typename std::vector<FieldT>::const_iterator scalar_end,
                                const size_t chunks)
