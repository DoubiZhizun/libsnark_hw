...
typename std::enable_if<std::is_same<T1, libff::alt_bn128_G1>().value || std::is_same<T2, libff::alt_bn128_G1>().value, knowledge_commitment<T1, T2>>::type kc_multi_exp_with_mixed_addition(const knowledge_commitment_vector<T1, T2> &vec,
                                                                ...
                                                                const size_t chunks)
{
    ...
    if (total >= (1lu << 18) && total <= (1lu << 27)) {
        ...
        while (index_it != vec.indices.end() && *index_it < max_idx) {
            vec_g[N] = value_it->g;
            vec_h[N] = value_it->h;
            scalar[N] = scalar_start[(*index_it) - min_idx];
            N++;
            ++value_it;
            ++index_it;
        }

        T1 result_g = libff::multi_exp_with_mixed_addition<T1, libff::alt_bn128_Fr, Method>(vec_g.begin(), vec_g.begin() + N, scalar.begin(), scalar.begin() + N, chunks);
        T2 result_h = libff::multi_exp_with_mixed_addition<T2, libff::alt_bn128_Fr, Method>(vec_h.begin(), vec_h.begin() + N, scalar.begin(), scalar.begin() + N, chunks);

        return libsnark::knowledge_commitment<T1, T2>(result_g, result_h);
    }
    ...
    while (index_it != vec.indices.end() && *index_it < max_idx)
    {
        ...
        const libff::alt_bn128_Fr scalar = *(scalar_start + scalar_position);
    ...
    return acc + libff::multi_exp<knowledge_commitment<T1, T2>, libff::alt_bn128_Fr, Method>(g.begin(), g.end(), p.begin(), p.end(), chunks);
}

template<typename T1, typename T2, typename FieldT, libff::multi_exp_method Method>
typename std::enable_if<!((std::is_same<T1, libff::alt_bn128_G1>().value || std::is_same<T2, libff::alt_bn128_G1>().value) && std::is_same<FieldT, libff::alt_bn128_Fr>().value), knowledge_commitment<T1, T2>>::type kc_multi_exp_with_mixed_addition(const knowledge_commitment_vector<T1, T2> &vec,
                                                                const size_t min_idx,
                                                                const size_t max_idx,
